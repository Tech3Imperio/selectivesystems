    ...
    <section>
      <div className="container pt-12 pb-20 overflow-hidden px-14 mt-10">
        <motion.h1
          ...
          whileInView={{ opacity: 1, x: 0 }}
          transition={{ duration: 1, delay: 0.3 }}
          className="text-3xl font-semibold font-poppins text-left pb-10 uppercase ml-4 md:ml-6 md:text-2xl lg:text-3xl xl:ml-14 xl:mb-4"
        >
          Our Products
        </motion.h1>
        <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-14 md:gap-14 lg:gap-16 lg:mt-7 lg:ml-12 xl:ml-[4rem] ">
          {ProductData.map((product) => (
            <motion.div
              variants={FadeLeft(product.delay)}
              initial="hidden"
